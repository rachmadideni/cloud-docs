...
**TL;DR**: Deploy aplikasi container ke Kubernetes managed AWS. EKS handle control plane (etcd, API server, scheduler), kamu manage workload pakai `kubectl` biasa. Bedanya dari [J8 Fargate](/id/journeys/j8-container): K8s untuk multi-service complex, ECS untuk single workload sederhana. Floci spin k3s cluster beneran di local.
...
Pakai image yang sudah di-push ke ECR di [J8](/id/journeys/j8-container#4-build-image-dan-push-ke-ecr).
