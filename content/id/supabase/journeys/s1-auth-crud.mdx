...
> **Catatan key:** `anon key` adalah nama legacy. Juli 2025 Supabase launch `publishable key` (format `sb_publishable_...`) sebagai pengganti, lebih aman karena bisa di-rotate. Lokal CLI masih emit anon key, jadi env var di atas pakai nama lama. Untuk project production baru, kamu mungkin lihat `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` di Dashboard. Fungsi identik dari sisi RLS. Detail di [Tools/Overview](/id/supabase/tools#catatan-nama-key-anon-vs-publishable).
...
- Track RLS sebagai code (test + CI gate biar tidak silent regression): [RLS Workflow](/id/supabase/tools/rls-workflow)
- Database deep + RLS pattern library: [Database service](/id/supabase/services/database)
- Next journey: **S2 File upload via Storage** (signed URL, image transform), work in progress.
- Service deep page: **Auth**, work in progress.
